...
export default function LobbyPage() {
  ...
  const nickname = useGameStore((s) => s.nickname) || localStorage.getItem('quiz_nickname')
  
  const [players, setPlayers] = useState([])
  const [quizTitle, setQuizTitle] = useState('')
  const [countdown, setCountdown] = useState(null)

  const { lastMessage, connectionStatus } = useGameSocket({ roomCode, role: 'player', nickname })
  ...
  useEffect(() => {
    ...
    switch (type) {
      case 'room_joined':
        ...
        break
      case 'player_joined':
        setPlayers((prev) => prev.includes(payload.nickname) ? prev : [...prev, payload.nickname])
        break
      case 'player_left':
  ...
  }, [lastMessage, roomCode, nickname, setRoom, setNickname, navigate])
        ...
        {countdown !== null ? (
          <div className="text-center space-y-8 animate-pulse">
            <div className="space-y-3">
              <h1 className="text-[10rem] font-black text-white italic leading-none tracking-tighter">{countdown}</h1>
              <p className="text-white/60 font-black uppercase tracking-[0.5em] text-xs">PREPARING FOR LIFT-OFF...</p>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-[3rem] p-10 shadow-2xl space-y-10 relative overflow-hidden border-b-8 border-blue-600/10">
            ...
            <div className="bg-blue-50 rounded-[2.5rem] p-8 space-y-6 shadow-inner border-2 border-blue-100/50">
              <div className="flex items-center justify-between border-b-2 border-blue-100 pb-4">
                <h2 className="text-blue-900 font-black italic uppercase tracking-tight text-xl">THE SQUAD</h2>
              ...
              {players.length === 0 ? (
                <div className="text-center py-16 space-y-4">
                  ...
                  <p className="text-blue-900/40 text-xs font-black italic uppercase tracking-widest">Waiting for squad members...</p>
                </div>
              ) : (
            ...
            <div className="text-center space-y-3">
              ...
              <div className="space-y-1">
                <p className="text-blue-900/30 font-black uppercase tracking-[0.3em] text-[9px]">Awaiting Host Instruction</p>
                <p className="text-blue-900 text-xs font-black italic uppercase tracking-tight">
                  Authenticated as <span className="text-blue-600 underline underline-offset-4">{nickname}</span>
                </p>
              </div>
            </div>
          </div>
